...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        x_embeded, cache_embed = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == 'rnn':
            h, cache_rnn = rnn_forward(x_embeded, h0, Wx, Wh, b)
        else:
            h, cache_lstm = lstm_forward(x_embeded, h0, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'rnn':
            for i in range(max_length):
                embedded, _ = word_embedding_forward(x, W_embed)
                h, _ = rnn_step_forward(embedded[:, 0], h, Wx, Wh, b)
            ...
            for i in range(max_length):
                embedded, _ = word_embedding_forward(x, W_embed)
                h, c, _ = lstm_step_forward(embedded[:, 0], h, c, Wx, Wh, b)
                s, _ = affine_forward(h, W_vocab, b_vocab)
                x = np.argmax(s, axis=1)
                captions[:, i] = x
                x = x.reshape(N, 1)
